...
class ShoppingSkill(object):
    ...
    def init_models(self):
        self.ffcc = tf.keras.models.load_model(
            os.path.join(basedir, self.model_name), compile=False
        )
        self.label_encoder = pickle.load(
            open(os.path.join(basedir, self.label_encoder_name), "rb")
        )
        self.infraset_model = torch.load(
            os.path.join(basedir, self.infraset_model_name)
        )
    ...
    def classify(self, text, cutoff=0.0):
        ...
        predicted_labels = self.get_labels_decoded(
